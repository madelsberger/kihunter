...
module.exports = function (properties, client, huntState, batchManager) {
  ...
  const multiguess = (properties.scoring.multiguess.toLowerCase() == 'true');
  const exact = (properties.scoring.match.toLowerCase() != 'closest');
        ...
        if (Number.isInteger(g) && g > 0 && g <= locationCount) {
          ...
          if (p = huntState[channel].guessers[user.username] ) {
            if (respondToBadGuess) {
              ...
              && (huntState[channel].guesses[g-1].offset == 0)) {
            if (respondToBadGuess) {
              client.say(channel,
                `Sorry, @${user.username}, ${g} was already guessed`, 0);
            }
          } else {
            recordGuess(huntState, channel, user, g);
      ...
      if ( isMod(channel, user) ) {
        ...
        if (Number.isInteger(g) && g > 0 && g <= locationCount) {
          if (huntState[channel].guesses[g-1]) {
            let w = huntState[channel].guesses[g-1].by;
  ...
  const openRanges = function (channel) {
    ...
    for(let i = 0; i < locationCount; i++) {
      if (guesses[i] && guesses[i].offset == 0) {
        r = null;
      } else {
        if (!r) {
          r = {start: i+1};
  ...
  const recordGuess = function (huntState, channel, user, g) {
    const start = (exact ? g : 1) - 1;
    const stop = (exact ? g : locationCount);
    let offset = (g - 1) - start;
    let nextOffset = x => x - 1;

    for (let i = start; i < stop; i++) {
      if (   !(huntState[channel].guesses[i])
          || huntState[channel].guesses[i].offset > offset) {
        huntState[channel].guesses[i] = {offset: offset, by: []};
        if (!offset) {huntState[channel].remaining -= 1};
      }

      if (huntState[channel].guesses[i].offset == offset) {
        huntState[channel].guesses[i].by.push(user.username);
      }

      if (!offset) {nextOffset = x => x + 1;}
